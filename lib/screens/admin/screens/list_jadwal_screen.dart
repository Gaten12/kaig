...
class ListJadwalController extends GetxController {
  ...
  Future<void> deleteJadwal(String jadwalId, String namaKereta) async {
    try {
      ...
      // Memanggil metode deleteJadwal yang sudah diperbarui
      await _adminService.deleteJadwal(jadwalId);
      Get.snackbar(
        'Berhasil',
        'Jadwal $namaKereta dan semua datanya berhasil dihapus.',
  ...
  Future<void> deletePastSchedules() async {
    ...
    final pastSchedules = allJadwal
        .where((jadwal) => jadwal.tanggalBerangkatUtama.toDate().isBefore(now))
        .toList();

    if (pastSchedules.isEmpty) {
      Get.snackbar(
        'Info',
        'Tidak ada jadwal yang sudah terlewat untuk dihapus.',
    ...
    Get.dialog(
      AlertDialog(
        ...
        content: Text(
            'Anda yakin ingin menghapus ${pastSchedules.length} jadwal yang sudah terlewat? Semua data kursi yang terkait juga akan dihapus.'),
        actions: [
          TextButton(
            onPressed: () => Get.back(),
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              ...
              try {
                await Future.forEach(pastSchedules, (jadwal) async {
                  try {
                    // Setiap panggilan ini sekarang akan menghapus jadwal + sub-koleksi
                    await _adminService.deleteJadwal(jadwal.id);
                    successCount++;
                  } catch (e) {
                    failCount++;
                    print("Gagal menghapus jadwal ${jadwal.id}: $e");
                  }
                });
                ...
                if (successCount > 0) {
                  Get.snackbar(
                    'Berhasil',
                    '$successCount jadwal yang sudah terlewat berhasil dihapus.',
                    backgroundColor: const Color(0xFF3B82F6),
                    colorText: Colors.white,
                    snackPosition: SnackPosition.BOTTOM,
                  );
                ...
                if (failCount > 0) {
                  Get.snackbar(
                    'Sebagian Gagal',
                    '$failCount jadwal gagal dihapus. Cek log untuk detail.',
                ...
                Get.snackbar(
                  'Error',
                  'Terjadi kesalahan saat penghapusan massal: $e',
                  backgroundColor: Colors.red,
                  colorText: Colors.white,
                  snackPosition: SnackPosition.BOTTOM,
                );
...
class ListJadwalScreen extends StatelessWidget {
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            tooltip: 'Hapus Jadwal Terlewat',
            onPressed: () {
              controller.deletePastSchedules();
            },
          ),
        ],
                ...
                Container(
                  ...
                  child: Obx(
                      ...
                      decoration: InputDecoration(
                        ...
                        prefixIcon:
                        const Icon(Icons.search_rounded, color: electricBlue),
                        filled: true,
                        fillColor: pureWhite,
                        ...
                        focusedBorder: OutlineInputBorder(
                          borderRadius: BorderRadius.circular(12),
                          borderSide:
                          const BorderSide(color: electricBlue, width: 2),
                        ),
          ...
          Expanded(
            child: StreamBuilder<List<JadwalModel>>(
              ...
              builder: (context, snapshot) {
                ...
                if (controller.allJadwal.length != jadwalList.length || !listEquals(controller.allJadwal, jadwalList)) {
                  controller.allJadwal.assignAll(jadwalList);
  ...
  Widget _buildJadwalCard(
      BuildContext context, JadwalModel jadwal, ListJadwalController controller) {
    ...
    try {
      ...
      tanggalTibaFormattedSimple =
          DateFormat('dd MMM', 'id_ID').format(jadwal.tanggalTibaUtama.toDate());
    } catch (e) {
            ...
            Row(
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    color:
                    const Color(0xFF374151).withAlpha((255 * 0.1).round()),
                    borderRadius: BorderRadius.circular(20),
                  ),
                  child: Row(
                    mainAxisSize: MainAxisSize.min,
    ...
    Get.dialog(
      AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              'Anda yakin ingin menghapus jadwal berikut? Seluruh data kursi terkait juga akan terhapus secara permanen.',
              style: TextStyle(
  ...
  // Helper untuk membandingkan list, dibutuhkan di StreamBuilder
